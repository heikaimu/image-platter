<template>
  <div class="p-4 bg-gray-100">
    <div class="rounded-lg overflow-hidden shadow-md">
      <label
        class="h-16 px-4 flex justify-between items-center border-b border-gray-200 border-opacity-50 bg-white relative mb-0"
      >
        <input type="file" class="hide-input" accept="image/*" @change="handleSelectFile">
        <span class="text-gray-800 text-base">Photo Select</span>
      ...
      <label class="h-16 px-4 flex justify-between items-center bg-white relative mb-0">
        <input type="file" class="hide-input" accept="image/*" camera @change="handleSelectFile">
        <span class="text-gray-800 text-base">Take A Photo</span>
    ...
    <div class="mt-2 rounded-lg overflow-hidden shadow-md">
      <label class="h-16 px-4 flex flex-col justify-center items-center bg-white mb-0" @click="handleOpenFilestack">
        <div class="flex justify-center items-center gap-2">
          <svg class="w-7" viewBox="0 0 20 20" fill="#3a559f">
            <path
          ...
          <svg class="w-7" viewBox="0 0 20 20">
            <defs>
              <linearGradient
                id="linearGradient"
                x1="6.71416922%"
          ...
          <svg class="w-7" viewBox="0 0 21 18">
        ...
        <span class="text-sm text-gray-800">Choose from Online Services</span>
      </label>
    </div>
  </div>
</template>
...
// 本地文件选择
async function handleSelectFile(e: Event) {
...
function handleOpenFilestack() {
  ...
  const options = {
    ...
    onOpen() {
      setTimeout(() => {
        const menus = document.getElementsByClassName('fsp-mobile-menu')
